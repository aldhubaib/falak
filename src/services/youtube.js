...
async function getApiKey(projectId) {
  if (projectId) {
    ...
    if (keys.length > 0) {
      // Pick a random key to distribute quota across multiple keys
      const entry = keys[Math.floor(Math.random() * keys.length)]
      return decrypt(entry.enc)
    }
  }
  throw new Error('YouTube Data API v3 key not configured for this project. Go to Settings and add it.')
}

async function ytFetch(endpoint, params, projectId) {
  const key = await getApiKey(projectId)
  ...
  if (!res.ok) {
    ...
    trackUsage({ projectId, service: 'youtube-data', action: endpoint, status: 'fail', error: msg })
    throw new Error(`YouTube API error: ${msg}`)
  }
  trackUsage({ projectId, service: 'youtube-data', action: endpoint, status: 'ok' })
  return res.json()
}

// ── Fetch channel metadata by handle or ID ────────────────────
async function fetchChannel(handleOrId, projectId) {
  ...
  const data = await ytFetch('channels', params, projectId)
...
async function fetchVideoMetadata(youtubeVideoId, projectId) {
...
async function fetchComments(youtubeVideoId, maxResults = 100, projectId) {
  try {
