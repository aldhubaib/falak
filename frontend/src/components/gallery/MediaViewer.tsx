            ...
            <div className="bg-background border-l border-border flex flex-col h-full min-h-0 overflow-hidden">
              ...
              <div className="px-4 py-3 border-t border-border flex gap-2 shrink-0">
                <button
                  ...
                  className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-card text-[12px] text-foreground font-medium hover:bg-card transition-colors"
                >
...
function MetadataSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2.5">
      <div className="text-[10px] text-muted-foreground font-mono uppercase tracking-widest">{title}</div>
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-3">
      <span className="text-[11px] text-muted-foreground font-mono shrink-0">{label}</span>
      <span className="text-[12px] text-muted-foreground text-right break-all leading-tight">{value}</span>
    </div>
  );
}
