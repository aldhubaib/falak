  ...
  const fetchChannels = () => {
    ...
    fetch("/api/channels", { credentials: "include" })
      ...
      .then((data: { channels: ApiChannel[] }) => {
        ...
        setChannels(ours.map(mapApiChannel));
      })
      .catch(() => setChannels([]))
    ...
    <div className="flex flex-col min-h-screen">
      <div className="h-12 flex items-center px-6 border-b shrink-0 max-md:px-4 border-[#151619]">
        <h1 className="text-sm font-semibold">
          Our Channels <span className="text-dim font-normal">({channels.length})</span>
        </h1>
      </div>
              ...
              {channels.map((ch) => (
                ...
                >
                  <div className="relative shrink-0">
                    ...
                    />
                    <span className="absolute top-0 right-0 w-1.5 h-1.5 rounded-full bg-success ring-[1.5px] ring-[#1e51e9]" />
                  </div>
