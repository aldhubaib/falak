...
function UrgencyBadge({ days }: { days: number }) {
  ...
  const label =
    days >= 7 ? "🔥 Closing fast" : days >= 3 ? `🔥 ${days}d open` : `${days}d open`;
  return (
...
function ChannelAvatar({ url, name, size = "w-5 h-5" }: { url: string | null; name: string; size?: string }) {
  ...
  return (
    <span
      className={`${size} rounded-full bg-elevated border border-border flex items-center justify-center text-[8px] font-bold text-dim uppercase`}
    >
      {name.slice(0, 2)}
    </span>
  );
...
export default function Brain() {
  ...
  const handleReExtract = async () => {
    ...
    try {
      const r = await fetch(`/api/brain/re-extract?projectId=${projectId}`, {
        method: "POST",
        credentials: "include",
      });
    ...
    stats.gapWins > 0
      ? Math.round(
          publishedVideos
            ...
            .reduce((a, v) => a + v.viewsRaw, 0) / stats.gapWins
        )
      : 0;
  ...
  const lateAvgViews =
    stats.lateCount > 0
      ? Math.round(
          publishedVideos
            .filter((v) => v.result === "late")
            .reduce((a, v) => a + v.viewsRaw, 0) / stats.lateCount
        )
      : 0;

  const advantage =
    lateAvgViews > 0 ? Math.round(((gapAvgViews - lateAvgViews) / lateAvgViews) * 100) : 0;

  const lastExtracted =
    competitorStories.length > 0 || untouchedStories.length > 0
      ? [...competitorStories, ...untouchedStories]
          .map((s) => s.date)
          .filter(Boolean)
          .sort()
          .at(-1) ?? "—"
      ...
      <div className="h-12 flex items-center justify-between px-6 border-b border-border shrink-0 max-lg:px-4">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[11px] text-dim font-mono hidden sm:block">
            The race is about stories — who finds the untouched case first wins
          </span>
        </div>
        <button
        ...
        <div className="flex gap-5 px-6 max-lg:px-4 py-6 max-lg:flex-col">
          ...
          <div className="flex-1 min-w-0 space-y-6">
            ...
            <div className="rounded-xl bg-background p-5">
              ...
              <div className="mb-6">
                ...
                <p className="text-[12px] text-dim mb-3">
                  Stories found in competitor research but never produced. Act before someone else
                  does.
                </p>
                {untouchedStories.length === 0 ? (
                  <p className="text-[12px] text-dim font-mono px-2">
                    No untouched windows detected — all recent competitor topics have been covered
                    or are older than 14 days.
                  </p>
                ) : (
                  <div className="space-y-1.5">
                    {untouchedStories.map((story) => {
                      ...
                      return (
                        <div
                          key={story.id}
                          className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-success/[0.04] border border-success/10 hover:bg-success/[0.07] transition-colors group"
                        >
                          <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-success/15 text-success shrink-0">
                            OPEN
                          </span>
                          <UrgencyBadge days={days} />
                          <span className="flex-1 text-[13px] text-right truncate font-medium">
                            {story.title}
                          </span>
              ...
              <div>
                ...
                >
                  <ChevronDown
                    className={`w-3.5 h-3.5 text-dim transition-transform ${
                      takenOpen ? "rotate-0" : "-rotate-90"
                    }`}
                  />
                ...
                {!takenOpen && (
                  <p className="text-[11px] text-dim ml-5.5 font-mono">
                    {competitorStories.length} stories taken · you'd be late on all of these
                  </p>
                )}
                {takenOpen && (
                  <>
                    <p className="text-[12px] text-dim mb-3 ml-5.5">
                      These exact stories are in competitor videos. If you make a video about them,
                      you are late.
                    </p>
                    <div className="space-y-1">
                      {competitorStories.map((story) => (
                        <div
                          key={story.id}
                          className="flex items-center gap-3 px-4 py-3 rounded-xl bg-surface hover:bg-elevated/60 transition-colors"
                        >
                          <span className="text-[9px] font-mono font-bold px-2 py-0.5 rounded-full bg-orange/15 text-orange shrink-0">
                            TAKEN
                          </span>
                          <div className="flex items-center gap-1 shrink-0">
                            {story.competitors.map((c, i) => (
                              <ChannelAvatar key={i} url={c.avatarUrl} name={c.name} />
                            ))}
                          </div>
                          <span className="text-[11px] text-dim font-mono shrink-0">
                            {story.competitors.length} competitor
                            {story.competitors.length > 1 ? "s" : ""} · {story.totalViews} views
                          </span>
                          <span className="flex-1 text-[13px] text-right truncate">
                            {story.title}
                          </span>
                          <span className="text-[11px] text-dim font-mono shrink-0">
                            {story.date}
            ...
            <div className="rounded-xl bg-background p-5">
              ...
              {publishedVideos.length === 0 ? (
                <p className="text-[12px] text-dim font-mono px-2">
                  No analysed videos from your channels yet. Run the pipeline on your channel
                  videos.
                </p>
              ) : (
                <div className="space-y-1">
                  {publishedVideos.map((video) => (
                    <div
                      key={video.id}
                      className="flex items-center gap-3 px-4 py-3.5 rounded-xl bg-surface hover:bg-elevated/60 transition-colors"
                    >
                      {/* Channel avatar */}
                      <TooltipProvider delayDuration={200}>
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <button
                              onClick={() =>
                                navigate(projectPath(`/channel/${video.channelId}`))
                              }
                              className="shrink-0 hover:opacity-80 transition-opacity"
                            >
                              <ChannelAvatar
                                url={video.channelAvatarUrl}
            ...
            <div className="text-[11px] text-dim font-mono text-center pb-4">
              Story database is per-workspace · refreshed from competitor video AI analysis
            </div>
          </div>
          ...
          <div className="w-[340px] max-lg:w-full space-y-5 shrink-0">
            ...
            <div className="rounded-xl bg-background p-5">
              ...
              <div className="flex rounded-xl overflow-hidden mb-4">
                <div className="flex-1 px-4 py-3 bg-surface text-center">
                  <div className="text-2xl font-semibold font-mono text-success">
                    {stats.gapWins}
                  </div>
              ...
              <div className="space-y-1.5 text-[12px]">
                <div className="text-dim">
                  Gap Win avg:{" "}
                  <span className="text-success font-semibold">{fmt(gapAvgViews)} views</span>
                </div>
                <div className="text-dim">
                  Late avg:{" "}
                  <span className="text-destructive font-semibold">{fmt(lateAvgViews)} views</span>
                </div>
                <div className="text-dim">
                  First-mover advantage:{" "}
                  <span className="text-foreground font-semibold">{advantage}%</span> more views
                </div>
              </div>
            </div>
            ...
            <div className="rounded-xl bg-background p-5">
              ...
              <p className="text-[12px] text-dim mb-3">
                Who's covering the most stories this period.
              </p>
              {competitorActivity.length === 0 ? (
                <p className="text-[12px] text-dim font-mono">No competitor data yet.</p>
              ) : (
                      ...
                      <div className="w-24 h-1.5 bg-elevated rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${Math.min(
                              100,
                              (c.count / (competitorActivity[0]?.count || 1)) * 100
                            )}%`,
                          }}
                        />
                      </div>
                      <span className="text-[11px] font-mono font-semibold text-foreground w-6 text-right">
                        {c.count}
                      </span>
                    </div>
                  ))}
            ...
            <div className="rounded-xl bg-background p-5">
              ...
              <p className="text-[12px] text-dim mb-3">
                Channels feeding this workspace's story database.
              </p>
              {competitorChannels.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2 mb-3">
                  {competitorChannels.map((ch) => (
                    <span
                      key={ch.id}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-surface text-[11px] font-medium text-sensor"
                    >
              ...
              <div className="text-[11px] text-dim font-mono">
                {competitorStories.length} stories already covered ·{" "}
                {untouchedStories.length} untouched windows found
              </div>
            </div>
            ...
            <div className="rounded-xl bg-background p-5">
              ...
              <p className="text-[12px] text-dim mb-4">
                Sent to Perplexity on every Fetch. Asks for stories NOT in the competitor
                database. Built from your gap wins.
              </p>
