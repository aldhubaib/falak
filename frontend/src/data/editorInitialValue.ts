...
/** Extract plain text from Yoopta value for export, AI, or backward compatibility. */
...
/** Compare two Yoopta values by content (for sync deduplication). */
export function areYooptaValuesEqual(
  a: YooptaContentValue | undefined | null,
  b: YooptaContentValue | undefined | null
): boolean {
  if (a === b) return true;
  if (!a || !b) return !a && !b;
  const textA = yooptaValueToScriptText(a);
  const textB = yooptaValueToScriptText(b);
  return textA === textB;
}
