...
| **AI — script writing** | OpenAI GPT-4.1-mini + Claude Sonnet + Claude Haiku | Two-pass pipeline: (1) `extract-facts` — researcher → fact sheet → user curation; (2) `generate-script` — dual writers (narrator ∥ storyteller) → editor merge → dual QA (accuracy ∥ quality, max 3 rounds) → editor final. Curated mode uses user-selected/pinned facts; full mode uses all. Bull queue for scalability. | `src/services/scriptPipeline.js`, `src/queue/scriptPipeline.js`, `src/services/openaiChat.js`, `src/routes/stories.js` |
